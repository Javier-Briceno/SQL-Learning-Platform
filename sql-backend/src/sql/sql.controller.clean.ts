...
async checkQueryMatchesTask(@Body() dto: CheckQueryDto): Promise<{ matches: boolean, aiAnswer: string }> {
  ...
  // Neue Route für vereinfachte Schema-Inspektion (nur Tabellen und Spalten mit Typen)
  @Get('inspect-schema/:dbName')
  async inspectDatabaseSchema(@Param('dbName') dbName: string) {
    return this.sqlService.inspectDatabaseSchema(dbName);
  }
  ...
  @Post('reset-database-copy/:database')
  @UseGuards(AuthGuard('jwt'))
  async resetDatabaseCopy(@Param('database') database: string, @Request() req): Promise<{ success: boolean, message: string }> {
    return this.sqlService.resetDatabaseCopy(database, req.user.id);
  }
